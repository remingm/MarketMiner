...
for filename in os.listdir(os.getcwd()):
	#if "newCols" not in filename and ".us.txt" in filename:
	if "newCols" not in filename and ".csv" in filename:	
		print filename

		try:

			colNames = ['D','Close','H','L','O','V']
			...
			#Indicators
			# BBupper, garbage, BBlower = talib.BBANDS(close, timeperiod = timeStats)
			# stockData['BBandUpper'] = BBupper
			# stockData['BBandLower'] = BBlower
			# stockData['CMOData'] = talib.CMO(close, timeperiod = timeStats)
			# stockData['DMIData'] = talib.DX(high=stockData['High'].values,low=stockData['Low'].values, close=close, timeperiod = timeStats)
			# stockData['MFI'] = talib.MFI(high=stockData['High'].values,low=stockData['Low'].values, close=close, volume=stockData['Volume'].values.astype(float), timeperiod = timeStats)
			# stockData['MA'] = talib.MA(close, timeperiod = timeStats)
			# stockData['STDDEV'] = talib.STDDEV(close, timeperiod = timeStats)
			...
			stockData.to_csv('newCols'+filename)
